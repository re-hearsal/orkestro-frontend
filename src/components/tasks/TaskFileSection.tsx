...
type TaskFileType = "PDF" | "PHOTO" | "AUDIO" | "VIDEO" | "OTHER" | "UNKNOWN";
...
export default function TaskFileSection({
  taskId,
  organizationId,
  fileIds,
  canManage,
  ...
  const uploadTaskFile = client.POST as unknown as (
    path: "/api/v1/organizations/{organizationId}/tasks/{taskId}/files",
    init: {
  ...
  const [menuTarget, setMenuTarget] = useState<TaskFileItem | null>(null);
  ...
  const [deleteTarget, setDeleteTarget] = useState<TaskFileItem | null>(null);
  const [deleting, setDeleting] = useState(false);

  const [uploading, setUploading] = useState(false);
  const [dragActive, setDragActive] = useState(false);

  const files = useMemo<TaskFileItem[]>(() => fileIds.map((id) => ({ id })), [fileIds]);

  const resolveFileType = (metadata?: FileMetadataDTO): TaskFileType => {
    if (!metadata?.fileType) return "UNKNOWN";
    return metadata.fileType as TaskFileType;
  };

  const getFileTypeLabel = (fileType: TaskFileType): string => {
  ...
  const openMenu = (event: MouseEvent<HTMLElement>, target: TaskFileItem) => {
  ...
  const handleDelete = async () => {
    ...
    try {
      const { error } = await client.DELETE(
        ...
        {
          params: { path: { organizationId, taskId, fileId: deleteTarget.id } },
          headers: { Authorization: `Bearer ${user.token}` },
        }
      );
      if (error) throw error;
  ...
  const uploadFile = async (file: File) => {
    ...
    if (fileIds.length >= 50) {
      showAlert(String(t("tasks.files.maxReached")), "error");
      return;
    }
    ...
    try {
      ...
      const { error } = await uploadTaskFile(
  ...
  const isMaxReached = fileIds.length >= 50;
  ...
  return (
    <Box>
      ...
      {canManage && isMaxReached && (
        <Typography
          variant="caption"
          sx={{ color: "#d32f2f", fontFamily: "Century Gothic, sans-serif", mb: 1, display: "block" }}
        >
          {t("tasks.files.maxReached")}
        </Typography>
      )}
